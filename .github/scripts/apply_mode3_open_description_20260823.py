...
path = Path('app/src/main/java/vn/nghetruyen/app/freesound/Mode3LibraryAssetMatcher.kt')
...
'''        val metadataTokens: Set<String>,
        val sections: Sections,''',
'IndexedTrack concepts')

replace_once('        val required = profile.requiredConcepts', '        val required = remoteRequiredConcepts(need)', 'remote required concepts')

regex_once(
r'''        val sections = sections\(track\.tagsCsv\)\n        val audibleText = if \(sections\.structured\) \{.*?        val indexed = IndexedTrack\(\n            key = key,\n            track = track,\n            metadataTokens = FreesoundAutoRequirementAggregator\.queryTokens\(track\.tagsCsv\),\n            sections = sections,\n            audibleConcepts = audibleConcepts\(audibleText\),\n        \)''',
...
'index local concepts')

regex_once(
r'''        val requiredConcepts = buildSet \{.*?        return NeedProfile\(\n            kind = need\.kind,\n            queryTokens = queryTokens,\n            coreToken = coreToken,\n            eventToken = eventToken,\n            requiredConcepts = requiredConcepts,\n            hints = hints,\n        \)''',
'''        return NeedProfile(
            kind = need.kind,
            queryTokens = queryTokens,
            coreToken = coreToken,
...
marker = '    private fun coreQueryToken(rawTokens: List<String>): String? {'
if text.count(marker) != 1:
    raise SystemExit('coreQueryToken marker not unique')
remote_helper = '''    private fun remoteRequiredConcepts(need: FreesoundAutoSearchNeed): Set<String> = buildSet {
...
'''
text = text.replace(marker, remote_helper + marker, 1)
...
regex_once(
r'''    private fun localSimilarity\(first: LocalText, second: LocalText\): Double \{.*?\n    private fun localNormalize\(value: String\): String =''',
...
'''    private const val DECISIVE_MIN_CONTEXT_SCORE = 0.48
    private const val DECISIVE_SEMANTIC_SCORE = 0.68''',
'decisive context')
replace_once('    private const val HARD_AVOID_CONFLICT = 0.72', '    private const val HARD_AVOID_CONFLICT = 0.66', 'avoid threshold')
replace_once('    private const val DECISIVE_SELECTION_FIT = 0.78', '    private const val DECISIVE_SELECTION_FIT = 0.68', 'decisive fit')
...
regex_once(
r'''    private val EMPTY_LOCAL_TEXT = LocalText\(.*?\)\n''',
