...
if "private const val TARGET_OVERLAP_RATIO" not in text:
    if "internal object AmbienceLoopTransitionPolicy {" not in text:
        raise SystemExit("SceneAmbienceController: ambience loop policy not found")
    start = text.index("internal object AmbienceLoopTransitionPolicy {")
    end = text.index("/**\n * Voice-first ambience bus", start)
    text = text[:start] + new_policy + "\n" + text[end:]
    print("PATCH SceneAmbienceController: file-duration continuous policy")
else:
    print("SKIP SceneAmbienceController: file-duration continuous policy already present")
...
print("PATCH XpkUnifiedNarrationPrompt: file duration is a runtime concern")
