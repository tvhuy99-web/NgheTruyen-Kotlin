...
public class Sonic {
    ...
    private static final short sincTable[] = {
        ...
        -2214, -2693, -3141, -3588, -4043, -4298, -4621, -4912, -5172, -5401,
    ...
    public void flushStream() {
        ...
        if(numOutputSamples > expectedOutputSamples) numOutputSamples = expectedOutputSamples;
        numInputSamples = 0;
        remainingInputToCopy = 0;
        numPitchSamples = 0;
    }

    public int samplesAvailable() { return numOutputSamples; }
    ...
    private void downSampleInput(short samples[], int position, int skip) {
        ...
        for(int i = 0; i < numSamples; i++) {
            value = 0;
            for(int j = 0; j < samplesPerValue; j++) value += samples[position + i*samplesPerValue + j];
    ...
    private boolean prevPeriodBetter(int minDiff, int maxDiff, boolean preferNewPeriod) {
        ...
        } else if(minDiff <= prevMinDiff) return false;
        return true;
    }
    ...
    private int findPitchPeriod(short samples[], int position, boolean preferNewPeriod) {
        ...
        if(prevPeriodBetter(minDiff, maxDiff, preferNewPeriod)) retPeriod = prevPeriod;
        else retPeriod = period;
        prevMinDiff = minDiff;
        prevPeriod = period;
        return retPeriod;
    }
    ...
    private int getSign(int value) { return value >= 0 ? 1 : -1; }
    ...
    private short interpolate(short in[], int inPos, int oldSampleRate, int newSampleRate) {
        ...
        for (int i = 0; i < SINC_FILTER_POINTS; i++) {
            ...
            int value = in[inPos + i*numChannels]*weight;
        ...
        if (overflowCount < 0) return Short.MIN_VALUE;
    ...
    private int skipPitchPeriod(short samples[], int position, float speed, int period) {
        int newSamples;
        if(speed >= 2.0f) newSamples = (int)(period/(speed - 1.0f));
        else {
            newSamples = period;
    ...
    private int insertPitchPeriod(short samples[], int position, float speed, int period) {
        int newSamples;
        if(speed < 0.5f) newSamples = (int)(period*speed/(1.0f - speed));
        else {
            newSamples = period;
    ...
    private void processStreamInput() {
        ...
        if(volume != 1.0f) scaleSamples(outputBuffer, originalNumOutputSamples, numOutputSamples - originalNumOutputSamples, volume);
    }
