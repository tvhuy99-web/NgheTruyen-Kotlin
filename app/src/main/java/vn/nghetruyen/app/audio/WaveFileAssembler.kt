...
/** Metadata for one RIFF/WAVE file generated by an Android TTS engine. */
...
/**
 * Validates and joins TTS-generated RIFF/WAVE segments without decoding them.
 *
 * All inputs must use exactly the same `fmt ` payload. Unknown RIFF chunks are
 * skipped safely and the output contains a normalized RIFF header followed by
 * one contiguous `data` chunk. The 32-bit RIFF size limit is enforced.
 */
object WaveFileAssembler {
    ...
    fun inspect(file: File): WaveSegment {
        ...
        RandomAccessFile(file, "r").use { input ->
            ...
            input.readUInt32Le() // Declared RIFF length; engines occasionally leave padding.
