...
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& $Python.Source @PythonPrefix ".\scripts\check_milestone2_source_platform.py"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& $Python.Source @PythonPrefix ".\scripts\check_vbook_static.py"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& $Python.Source @PythonPrefix ".\scripts\check_source_platform_android_static.py"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
...
$ExtraTasks = ""
if ($env:MILESTONE2_EXTRA_TASKS) {
    $ExtraTasks = $env:MILESTONE2_EXTRA_TASKS.ToLowerInvariant()
}
...
Write-Host "Milestone 2 complete build gates completed."
