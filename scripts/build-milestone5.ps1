...
& $Python.Source @PythonPrefix ".\scripts\check_roadmap_milestone5_playback_complete.py"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& $Python.Source @PythonPrefix ".\scripts\check_milestone5_foundation.py"
& $Python.Source @PythonPrefix ".\scripts\check_milestone4_complete.py"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& $Python.Source @PythonPrefix ".\scripts\check_audio_export_static.py"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& $Python.Source @PythonPrefix ".\scripts\check_p1_ui_static.py"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& $Python.Source @PythonPrefix ".\scripts\check_p2_ui_static.py"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& $Python.Source @PythonPrefix ".\scripts\check_p4_transfer_static.py"
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
...
$ExtraTasks = if ($env:MILESTONE5_EXTRA_TASKS) { $env:MILESTONE5_EXTRA_TASKS.ToLowerInvariant() } else { "" }
if ($ExtraTasks.Contains("connected")) {
    & ".\gradlew.bat" "--no-daemon" "--stacktrace" "connectedDebugAndroidTest"
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
}
if ($ExtraTasks.Contains("release")) {
    & ".\gradlew.bat" "--no-daemon" "--stacktrace" ":app:verifyReleaseSigning" "bundleRelease"
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
}

Write-Host "Milestone 5 complete build gates completed."
