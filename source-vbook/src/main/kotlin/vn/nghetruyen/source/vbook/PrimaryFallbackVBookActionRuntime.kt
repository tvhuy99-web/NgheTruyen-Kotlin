 ...
 *
 * Android Chromium handles synchronous Browser operations with its own cooperative replay boundary,
 * so engine selection must not guess from extension source text. Rhino fallback remains restricted
 * to explicit pre-execution Chromium-unavailable states where extension code could not have run.
 */
class PrimaryFallbackVBookActionRuntime(
    ...
    override fun execute(
        ...
        when (val result = primary.execute(manifest, resources, request)) {
